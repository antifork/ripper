...
init_all ()
{
  ...
  if (pcap_lookupnet (dev, &localnet, (u_int *) & routes[1][0], errbuf) < 0)
...
send_fake_rip_response ()
{
  ...
  udp = libnet_build_udp (RIP_PORT, RIP_PORT, LIBNET_UDP_H + sizeof (struct rip) + sizeof (struct rip_message) * w,	//RISCHIO INTEGER OVERFLOW
			  0,
			  buffer,
			  sizeof (struct rip) +
	      ...
	      const u_char * packet)
{
  ...
  printf (" HOST %s SPEAKS RIPv%i!!\n", in_ntoa (ip->saddr),
	  *(packet + sizeof_datalink (handle) + sizeof (struct iphdr) +
